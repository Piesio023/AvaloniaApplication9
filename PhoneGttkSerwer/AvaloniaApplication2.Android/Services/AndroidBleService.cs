...
public class AndroidBleService : IBleService
{
    ...
    public void StartServer(string serviceUuid, string charUuid, string newName)
    {
        try
        {
            var context = global::Android.App.Application.Context;
            _server = new BleServer(context);
            // ZMIANA 2: Przekazujemy parametry do metody Start w klasie serwera
            _server.Start(serviceUuid, charUuid , newName);
            Status = "Rozglaszanie...";
        }
        catch (Exception ex)
        {
...
public class BleServer
{
    ...
    public void Start(string serviceUuidStr, string charUuidStr, string newName)
    {
        if (_adapter == null || !_adapter.IsEnabled) return;

        SERVICE_UUID = UUID.FromString(serviceUuidStr);
        CHAR_UUID = UUID.FromString(charUuidStr);

        _originalDeviceName = _adapter.Name;
        bool success = _adapter.SetName(newName);
        Log.Info("BLE", $"Próba zmiany nazwy na {newName}: {(success ? "Sukces" : "Błąd/Odmowa")}");
